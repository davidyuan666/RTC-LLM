...
def main():
    ...
    p.add_argument('--input', '-i', required=True,default='audio_temp/temp_speech_record_20240819123751.wav')
...
'''
def inference_vocal(input_raw_audio):
    from scipy.io import wavfile
    import noisereduce as nr
        # 读取数据
    rate, data = wavfile.read(input_raw_audio)
        # 执行噪声抑制
    reduced_noise = nr.reduce_noise(y=data, sr=rate)
    wavfile.write(os.path.join(os.getcwd(),'output',"mywav_reduced_noise.wav"), rate, reduced_noise)

def inference_vocal_pydub(input_raw_audio):
    ...
    prediction = separator.separate_to_file(audio_descriptor, 'output.wav')

    # 这会在 'output/path' 目录下生成两个文件：'vocals.wav' 和 'accompaniment.wav'
...
if __name__ == '__main__':
    input_raw_audio =os.path.join(os.getcwd(),'..','audio_temp','temp_speech_record_20240819115411.wav')
    inference_vocal_pydub(input_raw_audio)